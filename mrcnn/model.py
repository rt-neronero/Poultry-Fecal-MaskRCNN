...
def conv_block(input_tensor, kernel_size, filters, stage, block,
    ...
    # print(f"Shape after shortcut conv: {shortcut.shape}")
...
class ShapeLogger(KL.Layer):
    def __init__(self, name="ShapeLogger"):
        super(ShapeLogger, self).__init__(name=name)

    def call(self, inputs):
        tf.print(self.name, tf.shape(inputs))
        return inputs
...
def resnet_graph(input_image, architecture, stage5=False, train_bn=True):
    ...
    x = KL.Activation('relu')(x)
    # print("Shape_After_Stage1:", ShapeLogger(name='Shape_After_Stage1')(x))
    ...
    # print("Shape_After_Stage2:", ShapeLogger(name='Shape_After_Stage2')(x))
    # Stage 3
    x = conv_block(x, 3, [128, 128, 512], stage=3, block='a', train_bn=train_bn)
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='b', train_bn=train_bn)
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='c', train_bn=train_bn)
    C3 = x = identity_block(x, 3, [128, 128, 512], stage=3, block='d', train_bn=train_bn)
    # print("Shape_After_Stage3:", ShapeLogger(name='Shape_After_Stage3')(x))
    # Stage 4
    ...
    C4 = x
    # print("Shape_After_Stage4:", ShapeLogger(name='Shape_After_Stage4')(x))
    # Stage 5
    if stage5:
        x = conv_block(x, 3, [512, 512, 2048], stage=5, block='a', train_bn=train_bn)
        x = identity_block(x, 3, [512, 512, 2048], stage=5, block='b', train_bn=train_bn)
        C5 = x = identity_block(x, 3, [512, 512, 2048], stage=5, block='c', train_bn=train_bn)
        # print("Shape_After_Stage5:", ShapeLogger(name='Shape_After_Stage5')(x))
    else:
        C5 = None
...
class DataGenerator(KU.Sequence):
    ...
    def __init__(self, dataset, config, shuffle=True, augmentation=None,
                 random_rois=0, detection_targets=False, validation_mode=False):
    ...
    def __getitem__(self, idx):
        ...
        if self.validation_mode:
            return batch_images, [batch_gt_class_ids, batch_gt_boxes]
        else:
...
class MemoryUsageCallback(keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs=None):
        max_mem_usage = max(memory_usage(-1, interval=0.1, timeout=1))
        print(f'Max memory usage during epoch {epoch + 1}: {max_mem_usage} MiB')
        if logs is not None:
            logs['memory_usage'] = max_mem_usage
...
class MaskRCNN(object):
    ...
    def build(self, mode, config):
        ...
        from tensorflow.keras.layers import Input, Add, Conv2D, UpSampling2D, MaxPooling2D
        ...
        # print("P5 FLOPs:", get_flops(P5))
        P4 = KL.Add(name="fpn_p4add")([
            KL.UpSampling2D(size=(2, 2), name="fpn_p5upsampled")(P5),
            KL.Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (1, 1), name='fpn_c4p4')(C4)])
        # print("P4 FLOPs:", get_flops(P4))
        P3 = KL.Add(name="fpn_p3add")([
            KL.UpSampling2D(size=(2, 2), name="fpn_p4upsampled")(P4),
            KL.Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (1, 1), name='fpn_c3p3')(C3)])
        # print("P3 FLOPs:", get_flops(P3))
        P2 = KL.Add(name="fpn_p2add")([
            KL.UpSampling2D(size=(2, 2), name="fpn_p3upsampled")(P3),
            KL.Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (1, 1), name='fpn_c2p2')(C2)])
        # print("P2 FLOPs:", get_flops(P2))
    ...
    def train (self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        layer_regex = {
            ...
            "5+": r"(res5.*)|(bn5.*)|def train(mrcnn\_.*)|(rpn\_.*)|(fpn\_.*)",
            # All layers
            "all": ".*",
        }
        ...
        callbacks = [
            ...
            MemoryUsageCallback()  # Add the memory usage callback here
        ]
